...
{
	0,//ºÊ»›Œﬁ ccµƒ«π£¨10A,20171227»•≥˝ ≤ª∑˚∫œπ˙±Í∂º≤ª≥‰µÁ
	0,
	10,
	0,
	16,
...
//	uint16	actChrCurrentTemp = 0;
	...
	//OBCTag.actChrCurrent = actChrCurrentTemp>40?actChrCurrentTemp-40:0;//µÁ¡˜º∆À„°£
	L3_Dcdc_IoutFilter_Ct_u16 =  LPF((uint32)L3_Dcdc_Iout_Ct_u16,(uint32)L3_Dcdc_IoutFilter_Ct_u16,100,L3_Dcdc_IoutFilter_Tau_Ct_u16);
	
	//OBCTag.actChrCurrent =(uint16)((DCDC_AD_CONVERT_100doubleCURRENT(L3_Dcdc_Iout_Ct_u16)<<4)/100);//µÁ¡˜º∆À„°£
	OBCTag.actChrCurrent =(uint16)((DCDC_AD_CONVERT_100doubleCURRENT(L3_Dcdc_IoutFilter_Ct_u16)<<4)/100);//µÁ¡˜º∆À„°
	//OBCTag.actChrCurrent =(uint16)((DCDC_AD_CONVERT_1000doubleCURRENT(L3_Dcdc_IoutFilter_Ct_u16)<<4)/1000);//µÁ¡˜º∆À„°
	//OBCTag.actChrVolt = (uint16)(((uint32)L3_Dcdc_Vout_Ct_u16*1000)>>10);//µÁ—πº∆À„
	//OBCTag.actChrVolt = DCDC_AD_CONVERT_VOLT(L3_Dcdc_Vout_Ct_u16)*2;//µÁ—πº∆À„
	OBCTag.actChrVolt = (uint16)((DCDC_AD_CONVERT_100doubleVOLT(L3_Dcdc_Vout_Ct_u16)<<1)/100);//µÁ—πº∆À„
	
	OBCTag.ChrOpMode = stSysFSMCtrl.ucCurrentState;
	//OBCTag.ChrTemp 
	//OBCTag.FaultID
	if(bms_can_available && faultCodeTag.bit.bLV12VFault == FALSE)
	{
		OBCTag.bErrorCOM = L3_S_BMSMsgTimeOut_Uls_G_u08 || (!OBCTag.bFilterWorkCMD);//BMS÷°Õ®–≈ «∑Ò’˝≥£
	}
	else
	{
		OBCTag.bErrorCOM = FALSE;//BMS÷°Õ®–≈ «∑Ò’˝≥£
	}
	...
	OBCTag.bFaultHW = faultCodeTag.bit.bHWDCDC_OVP || faultCodeTag.bit.bHWDCDC_OCP || faultCodeTag.bit.bHWPFC_OVP || faultCodeTag.bit.bHWPFC_OCP||faultCodeTag.bit.bStartShortCircuit;
...
void CanTxFramePack1(void)
{
	...
	//OBCTag.ACRMSVolt  = (uint16)(((float)AC_AD_CONVERT_VOLT(L5_AC_Volt_fast_max_buff))*2*0.707);//AC”––ßµÁ—πº∆À„
	//OBCTag.ACRMSVolt  = (uint16)(((float)AC_AD_CONVERT_VOLT(L5_AC_Volt_fast_max_buff))*0.707);//AC”––ßµÁ—πº∆À„
	OBCTag.ACRMSVolt  = (uint16)(707*(AC_AD_CONVERT_VOLT(L5_AC_Volt_fast_maxBuffCPS))/1000);//AC”––ßµÁ—πº∆À„

	//OBCTag.ACRMSVolt  = (uint16)(((float)AC_AD_CONVERT_VOLT(829))*2*0.707);//AC”––ßµÁ—πº∆À„

	//OBCTag.ACRMSVolt  = 220*2;//AC”––ßµÁ—πº∆À„
	//mCanTxFrameTag1.bit.CHR02_CCState = 0;


	//OBCTag.ACRMSVolt  = (uint16)(((float)AC_AD_CONVERT_VOLT(L3_AD0_AC_Volt_fast_Ct_u16))*2);//AC µº ÷µ
	
	OBCTag.PFCVolt = (uint16)((PFC_AD_CONVERT_1000double_VOLT(L3_I_AD_PFC_slow_Ct_u16)<<1)/100);//µÁ—πº∆À„
	...
	if(faultCodeTag.bit.bFaultCP == 0)
	{
		mCanTxFrameTag1.bit.CHR02_CPState = CP_NORMAL;
	}
	else
	{
		if(OBCTag.bFilterCPVoltState != CP_NORMAL)
		{
			mCanTxFrameTag1.bit.CHR02_CPState = OBCTag.bFilterCPVoltState;
		}
		
		if(OBCTag.bFilterCPDutyState != CP_NORMAL)
		{
			mCanTxFrameTag1.bit.CHR02_CPState = OBCTag.bFilterCPDutyState;
		}
		
		if(OBCTag.bFilterCPFrqState != CP_NORMAL)
		{
			mCanTxFrameTag1.bit.CHR02_CPState = OBCTag.bFilterCPFrqState;
		}
	}
...
int Filter() 
{
	int i;
	int filter_sum = 0;
	filter_buf[FILTER_N] = Get_AD(); //AD◊™ªªµƒ÷µ∏≥∏¯ ˝◊È◊Ó∫Û“ª∏ˆ÷µ
